---
title: "Climate Change: Relationship between Cyclone Intensity, Sea Ice Extent, and Awareness of Climate Change Definitions"
author: "Group 33"
...
This report explores the relationship between cyclone intensity, sea ice extent, and climate awareness. Our analysis shows no strong correlation between cyclone intensity and sea ice extent, with cyclones of varying strengths occurring across a broad range of ice levels. Higher wind speeds show a slight association with increased climate awareness, particularly among those with moderate to high awareness, while low awareness is more common in areas with weaker cyclones. These findings suggest that cyclone experience may influence climate awareness slightly, but other factors likely play a larger role. Future work will incorporate additional environmental and socio-economic variables to expand insights, though our current sample size and geographic scope are limited.

# Introduction

Climate change is something that has been studied. In recent years, significant attention has been paid to the relationship between climate change and extreme weather events, such as cyclones melting sea ice levels. Understanding how these factors interact, along with how people respond to them in terms of climate awareness, can provide valuable insights into the broader impacts of climate change on our planet and actions to address these issues.

In this report, we will examine the relationship between cyclone intensity (measured by maximum wind speed), sea ice extent, and climate awareness levels. The data used for this analysis includes yearly cyclone strength metrics, sea ice extent observations and survey data measuring climate change awareness across countries. We will employ various cleaning, summary statistics, combining, and exploratory data analysis techniques, including hexbin plots, scatter plots and violin plots, to visualize the distribution, trends, and possible correlations between the two variables.

By the end of this report, we aim to investigate whether hurricanes and sea ice levels have any impact on each other (under the context of increasing climate change level). Furthermore, we want to examine the relationship between hurricane strength and levels of climate awareness, whether that direct exposure to extreme weather may be correlated with higher awareness. However, we consider that there are limitations in our standardization process, where many adjustments need to be made to combine the data.
...
To combine the climate awareness and cyclone data, we first mapped each cyclone's geographical coordinates (latitude and longitude) to a specific country. Using spatial mapping, we converted cyclone coordinates into country names by overlaying them on a world map. This allowed us to assign each cyclone event to its respective country. Next, we calculated the average maximum wind speed for each country based on these events, creating a summary of cyclone intensity by location. We then standardized country names in both datasets to ensure the maximum number of matching countries. Finally, we merged this summarized cyclone data with the climate awareness data by matching countries and transformed the climate awareness levels into a long format to facilitate comparison across different awareness categories.
...
kable(head(cyclones_climate_awareness,n=10)) |> kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover", "condensed"))
```

# Exploratory Data Analysis

To achieve our goals, we explored the data by using summary statistics to measure variability and visualization tools to assess the relationship between cyclone characteristics, sea ice extent, and climate change awareness levels across regions.

We explored many aspects of the data, but will demonstrate three. These are the lack of strong relationship between cyclone intensity and sea ice extent, a slight positive correlation between higher cyclone wind speeds and increased climate change awareness, and the distribution of climate awareness levels, where high awareness is concentrated around moderate cyclone intensities, and low or ambivalent awareness is more common in areas with lower wind speeds.
...
The next insight that we found is shown in \@ref(fig:insight2). Overall, there is a weak or mixed correlation between cyclone wind speed and climate change awareness levels, indicating that direct cyclone experience does not strongly influence awareness. However, there is a slight upward trend for high and moderate awareness level, suggesting that people exposed to higher wind speeds may be marginally more aware of climate change. Conversely, groups having no awareness or refuse to answer tend to decline as wind speed increases, implying that low awareness is more common in areas with lower cyclone intensity. The little awareness category remains stable, showing little sensitivity to cyclone wind speed.

```{r insight2, fig.height=4, fig.width=6, fig.cap="Higher cyclone wind speeds correlate with increased climate awareness, while low awareness remains stable or declines."}
...
Finally, \@ref(fig:insight3) shows that individuals with mid to moderate climate awareness show a broad distribution across various wind speeds, indicating little influence from cyclone intensity. In contrast, those with strong awareness are concentrated around moderate wind speeds, suggesting a link between notable cyclone experience and higher awareness. Lack of awareness is more common in regions with lower wind speeds, potentially due to limited cyclone exposure, while those who refuse to answer are also centered around lower wind speeds, hinting at possible ambivalence toward climate change in less cyclone-prone areas.

```{r insight3, fig.height=4, fig.width=6, fig.cap="Higher cyclone wind speeds are somewhat associated with stronger climate awareness, while lower wind speeds correspond to a lack of awareness or refusal to acknowledge climate change."}
ggplot(cyclones_climate_awareness, aes(x = value, y = mean_wind,fill=level)) +
  geom_violin() +
  facet_wrap(~level) +
  labs(
    title = "Climate Awareness Levels Across Mean Wind Speeds",
    x = "Mean Wind",
    y = "Awareness Value"
  ) +
  theme_minimal() + 
  scale_fill_manual(values = c("#006ee6","#0055b3","#003d80","#00254d","#000000"))

```

# Conclusion and Future Work

Our analysis reveals that there is no strong correlation between cyclone intensity and sea ice extent, as cyclones of all strengths occur across a range of ice extents. There is a common concentration at moderate ice levels and wind speeds. Furthermore, while higher cyclone wind speeds show a slight positive correlation with increased climate change awareness, particularly among individuals with moderate to strong awareness levels, those with low or no awareness tend to be concentrated in regions with lower cyclone intensity. These findings suggest that hurricane and changing sea ice extent might be weather events that are happening separately, not the reaction chain of climate change. Moreover, it also indicates that direct cyclone experience may have a slight impact on climate awareness, but factors beyond cyclone intensity are likely more influential in shaping public perceptions of climate change.

For future analysis, we plan to enhance the model by incorporating additional environmental variables, such as temperature and precipitation patterns to gain more insights into the way they are interacting or impacting each other. Expanding the model to consider socio-economic and educational factors could also provide a richer context for interpreting climate awareness trends, making our insights more actionable for education programs.

We still have some limitations, especially the process of standardization to combine the data, which reduce our sample significantly. In detail, when combining data from cyclones intensity data and climate awareness data, our sample seem to be too small with only 46 matching countries. This limited sample size restricts the generalizability of our findings and may not capture the full diversity of climate experiences across different regions. Beside, we also cannot fully cover the relationship between Antarctic sea ice extent and hurricane strength (due to the basins are both in North Hemisphere), but only for Arctic.

# References

1.  jdjgh
2.  *NHC Data Archive*. (2018). Noaa.gov. <https://www.nhc.noaa.gov/data/>
3.  
