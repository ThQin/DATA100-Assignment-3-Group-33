...
We explored many aspects of the data, but will demonstrate three. These are **the lack of strong correlation between cyclone intensity and sea ice extent**, **the temporal trends in cyclone intensity and sea ice extent across decades**, and **the distribution of cyclones across varying sea ice extents with no clear correlation**.

t aspect that we found inter\@ref(fig:insight1). The scatter plot shows that cyclone intensity (max wind speed) remains fairly consistent across varying levels of sea ice extent. There is no observable trend, either positive or negative, between cyclone wind speed and ice extent (the regression line is almost horizontal). This indicates that sea ice extent does not strongly influence the strength of cyclones in this data set.
...
The next insight that we found is shown in \@ref(fig:insight2). There are changes in cyclone intensity and sea ice extent. In particular, cyclone intensity (max wind speed) seems to increase over time, especially from the 2000s onwards with a broader range of wind speeds. Meanwhile, sea ice extent appears to have decreased over the decades, particularly in recent years (2020s). However, the smooth lines of plots for each decade are not consistent with other in direction (some are negative and some are positive) and also are not strong enough (with small slopes of the lines).
...
cyclones_ice_decade <- cyclones_ice_yearly |> 
  filter(!is.na(category)) |> 
        mutate(decade = floor(year / 10) * 10)
    
ggplot(cyclones_ice_decade) +
    aes(x = avg_ice_extent, y = max_wind,colour=category,na.rm=TRUE) +
    ...
    labs(
        x = "Average Sea Ice Extent (million km²)",
        y = "Cyclone Max Wind (knots)",
        color = "Cyclone Category",
        title = "Cyclone Intensity vs. Sea Ice Extent by Cyclone Category",
        subtitle = "Comparing Intensity and Sea Ice Levels Across Decade"
    ) +
    theme_minimal()

```

Finally, \@ref(fig:insight3) shows that cyclones of varying intensities (from TD to Category 5) are observed across the full range of sea ice extents (from 12 to 16 million km²). For most cyclone categories, the peak intensity remains around 12–15 million km² of ice extent, but there is no clear trend indicating that higher or lower ice extents are linked to stronger or weaker cyclones.

Finally, \@ref(fig:insight3) with the hex bins shows that there is no directional trend, suggesting that cyclone intensity is not directly related to sea ice extent. Cyclones of varying intensities occur across the spectrum of ice extents. The concentration around 12–15 million km² of ice extent with moderate wind speeds suggests that cyclones are most frequently observed under these conditions.

```{r insight3, fig.height=4, fig.width=6, fig.cap="There is indication of common conditions for observations, but still no strong correlation"}
ggplot(cyclones_ice_yearly) +
    aes(x = avg_ice_extent, y = max_wind) +
    geom_hex(bin=20) +
    labs(
...
ggplot(cyclones_ice_decade, aes(x = avg_ice_extent, fill = category)) +
  geom_density(alpha = 0.5) +
  facet_wrap(~category) +
  labs(title = "Density Plot of Cyclone Intensity and Sea Ice Extent",
       x = "Average Sea Ice Extent (million km²)",
        y = "Cyclone Max Wind (knots)") +
  theme_minimal()
