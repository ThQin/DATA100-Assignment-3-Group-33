...
aware_covid <- aware_pop |> 
  inner_join(covid_2020_match_country,
             by = "country") |> 
...
ice_extent_yearly <- ice_extent_daily |>
    ...
    group_by(year, region) |>
    summarise(
        avg_ice_extent = mean(ice_extent, na.rm = TRUE),
        min_ice_extent = min(ice_extent, na.rm = TRUE),
        max_ice_extent = max(ice_extent, na.rm = TRUE),
        .groups = 'drop'
    )
cyclones_ice_yearly <- cyclones_data |>
    inner_join(ice_extent_yearly, by = c("year", "region"))


print(cyclones_ice_yearly)
...
summary_stats_yearly <- cyclones_ice_yearly |>
    summarise(
        ...
        mean_avg_ice_extent = mean(avg_ice_extent, na.rm = TRUE),
        median_avg_ice_extent = median(avg_ice_extent, na.rm = TRUE),
        sd_avg_ice_extent = sd(avg_ice_extent, na.rm = TRUE)
    )

summary_stats_yearly |>
    gt() |>
    tab_header(
        title = "Summary Statistics for Cyclone and Yearly Sea Ice Extent Data",
        subtitle = "Minimum, Maximum, Mean, Median, and Standard Deviation"
    ) |>
    fmt_number(
        columns = everything(),
        decimals = 2
    ...
    cols_label(
        ...
        min_max_wind = "Min Wind Speed",
        max_max_wind = "Max Wind Speed",
        mean_max_wind = "Mean Wind Speed",
        median_max_wind = "Median Wind Speed",
        sd_max_wind = "Wind Speed SD",
        min_avg_ice_extent = "Min Avg Ice Extent",
        max_avg_ice_extent = "Max Avg Ice Extent",
        mean_avg_ice_extent = "Mean Avg Ice Extent",
        median_avg_ice_extent = "Median Avg Ice Extent",
        sd_avg_ice_extent = "Avg Ice Extent SD"
    ) |>
    tab_style(
...
PLOT1: 
```{r}
cyclone_ice_summary_general <- cyclones_ice_yearly |> 
    group_by(year, region) |>
    summarise(
        mean_max_wind = mean(max_wind, na.rm = TRUE),
...
ggplot(cyclone_ice_summary_general) +
    aes(x = year) +
    geom_line(aes(y = mean_max_wind, color = "Mean Max Wind (knots)"), size = 1) +
    geom_line(aes(y = mean_ice_extent, color = "Mean Ice Extent (million km²)"), size = 1, linetype = "dashed") +
    facet_wrap(~ region) +
    labs(
        x = "Year",
        y = "Mean Value",
        title = "Yearly Mean Cyclone Intensity and Sea Ice Extent",
        subtitle = "Broad Trends in Arctic and Antarctic Regions",
        color = "Metric"
    ) +
    theme_minimal()
...
```{r}
cyclone_ice_summary <- cyclones_ice_yearly |>
    group_by(year, region, category) |>
    summarise(
        avg_max_wind = mean(max_wind, na.rm = TRUE),
        avg_ice_extent = mean(avg_ice_extent, na.rm = TRUE),
        .groups = 'drop'
    )

ggplot(cyclone_ice_summary) +
    aes(x = avg_ice_extent, y = avg_max_wind, color = category) +
    geom_point(alpha = 0.7, size = 2) +
    facet_wrap(~ region) +
    labs(
        x = "Average Sea Ice Extent (million km²)",
        y = "Average Cyclone Max Wind (knots)",
        color = "Cyclone Category",
        title = "Cyclone Intensity vs. Sea Ice Extent by Region",
        subtitle = "Analysis of Cyclone Categories and Sea Ice Levels"
    ) +
    theme_minimal()
...
cyclones_ice_yearly |> 
  ggplot() +
    aes(x = category, y = max_wind, fill = region) +
    geom_boxplot() +
    labs(
        x = "Cyclone Category",
        y = "Cyclone Max Wind (knots)",
        fill = "Region",
        title = "Cyclone Intensity Distribution by Category and Region",
        subtitle = "Comparison of Cyclone Strengths across Arctic and Antarctic Regions"
    ) +
    theme_minimal()
