...
if (!"region" %in% colnames(cyclones_data)) {
    cyclones_data <- cyclones_data |>
        mutate(region = if_else(lat >= 0, "Arctic", "Antarctic"))
}

cyclones_ice_data <- cyclones_data %>%
    inner_join(ice_extent_daily, by = c("date", "region"))

print(cyclones_ice_data)
...
summary_stats <- cyclones_ice_data |>
    summarise(
        ...
        mean_ice_extent = mean(ice_extent, na.rm = TRUE),
        median_ice_extent = median(ice_extent, na.rm = TRUE),
        sd_ice_extent = sd(ice_extent, na.rm = TRUE)
    )

summary_stats |>
    gt() |>
    tab_header(
        title = "Summary Statistics for Cyclone and Sea Ice Extent Data",
    ...
    cols_label(
        ...
        min_ice_extent = "Min Ice Extent",
        max_ice_extent = "Max Ice Extent",
        mean_ice_extent = "Mean Ice Extent",
        median_ice_extent = "Median Ice Extent",
        sd_ice_extent = "Ice Extent SD"
    ) |>
    tab_style(
...
PLOT1: 
```{r}
cyclone_ice_summary_general <- cyclones_ice_data |>
    group_by(year, region) |>
    summarise(
        mean_max_wind = mean(max_wind, na.rm = TRUE),
        mean_ice_extent = mean(ice_extent, na.rm = TRUE),
...
```{r}
ggplot(cyclones_ice_data) +
    aes(x = ice_extent, y = max_wind, color = category) +
    geom_point(alpha = 0.7) +
    facet_wrap(~ region) +
    labs(
        x = "Sea Ice Extent (million km²)",
        y = "Cyclone Max Wind (knots)",
        color = "Cyclone Category",
        title = "Cyclone Intensity vs. Sea Ice Extent by Region",
        subtitle = "Impact of Cyclone Category and Sea Ice Levels"
    ) +
    theme_minimal()
...
cyclones_ice_data |> 
  ...
  labs(
        ...
        y = "Cyclone Max Wind (knots)",
        fill = "Region",
        title = "Cyclone Intensity by Category and Region",
        subtitle = "Comparative Distribution of Cyclone Strengths"
    ) +
    theme_minimal()
